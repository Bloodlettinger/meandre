...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for item in orm['auth.Group'].objects.all():
            orm.MembershipRole.objects.create(
                title=item.name, title_ru=item.name)
        orm['auth.Group'].objects.all().delete()
        db.rename_column('storage_membership_role', 'customgroup_id', 'membershiprole_id')

    def backwards(self, orm):
        for item in orm.MembershipRole.objects.all():
            orm['auth.Group'].objects.create(
                name=item.title)
        orm.MembershipRole.objects.all().delete()
        db.rename_column('storage_membership_role', 'membershiprole_id', 'customgroup_id')
