...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for project in models.Project.objects.all():
            print '> %s' % project
            for record in project.membershipstaff_set.all():
                for role in record.role.all():
                    created = False
                    params = dict(project=record.project, role=role)
                    try:
                        obj = models.Membership.objects.get(**params)
                    except models.Membership.DoesNotExist:
                        obj = models.Membership(**dict(params, position=0))
                        created = True
                    print '\t%s %s: %s' % (
                        '=' if created else '+',
                        role,
