...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for item in orm.Project.objects.all():
            item.object_square2 = u'%.02f' % item.object_square
            item.price_full2 = u'%.02f' % item.price_full
            item.exchange_rate2 = u'%.02f' % item.exchange_rate
            item.price_average2 = u'%.02f' % item.price_average
            item.productivity2 = u'%.02f' % item.productivity
            item.save()

    def backwards(self, orm):
        for item in orm.Project.objects.all():
            item.object_square = item.object_square2
            item.price_full = item.price_full2
            item.exchange_rate = item.exchange_rate2
            item.price_average = item.price_average2
            item.productivity = item.productivity2
            item.save()
