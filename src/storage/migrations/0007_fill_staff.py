...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for item in models.StaffPerson.objects.all():
            print '> %s' % item
            pk = item.staff_ptr.pk
            try:
                obj = models.Staff.objects.get(pk=pk)
            except models.Staff.DoesNotExist:
                print '\tError: Staff PK %i is unknown!' % pk
            else:
        ...
        for item in models.StaffCompany.objects.all():
            print '> %s' % item
            pk = item.staff_ptr.pk
            try:
                obj = models.Staff.objects.get(pk=pk)
