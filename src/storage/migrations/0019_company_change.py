...
from src.storage.models import Membership
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for item in Membership.objects.filter(company__isnull=False):
            try:
                user = CustomUser.objects.get(company__name=item.company.name)
            except CustomUser.DoesNotFound:
                print 'Cannot find user with the following company:', item.company.name
            else:
                item.user = user
                item.save()
    ...
    models = {
        ...
        'storage.project': {
            ...
            'registered': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
