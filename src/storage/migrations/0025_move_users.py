...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for user in CustomUser.objects.filter(project__isnull=False).distinct():
            print user
            staff = None
            if user.last_name == '' and user.company:
                models.StaffCompany(
                ...
                print '-' * 20
                print 'Unknown user >>> %s' % user.username
                print '\t Skipping...'
        ...
        for project in models.Project.objects.all():
            ...
            for membership in project.membership_set.all():
                ...
                if not user:
                    print '\t### Membership has no user, skipping...'
                    continue
                try:
                    if user.last_name == '' and user.company:
                        staff = models.StaffCompany.objects.get(
                            title=user.company.name)
                    else:
                        staff = models.StaffPerson.objects.get(
                            last_name=user.last_name,
                            first_name=user.first_name)
                except ObjectDoesNotExist:
                    print 'ERROR:', user

                obj = models.MembershipStaff(project=project, staff=staff)
                obj.save()
