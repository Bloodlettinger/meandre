...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for user in CustomUser.objects.filter(company__isnull=False).exclude(last_name=''):
            print 'User: %s' % user
            try:
                obj = models.StaffPerson.objects.get(last_name=user.last_name, first_name=user.first_name)
            except models.StaffPerson.DoesNotExist:
                print 'ERROR:', user
                raise
            else:
