...
from src.users.models import CustomUser, Company
    ...
    def forwards(self, orm):
        for item in Membership.objects.filter(company__isnull=False):
            try:
                ...
                if item.url:
                    company.site = item.url
                company.save()
            except IntegrityError:
                company = Company.objects.get(name=item.company.name)

            try:
                user = CustomUser.objects.create_user(
                    username=slugify(item.company.name)[:30]
                )
